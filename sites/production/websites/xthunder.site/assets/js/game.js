...
function canPlace(type, x, y) {
  if (!BUILD[type] || isCoreTile(x, y)) return false;
  if (state.grid[y][x]) return false;
  if (type === "trap") return true;
  if (state.gold < BUILD[type].cost) return false;
  return true;
}
...
function updateHUD() {
  ...
  document.querySelectorAll(".tool[data-tool]").forEach((btn) => {
    ...
    btn.disabled = state.phase === "fight" && state.fightActive;
    btn.querySelector(".tool-cost").textContent = state.gold < cost ? `${cost} ✗` : `${cost}`;
  });
}
