#!/usr/bin/env python3
"""
Publish dreamscape episodes to live site using WP-CLI
"""
...
def publish_episodes():
    """Publish the 10 dreamscape episodes to the live site"""
...
Authority building takes time but creates significant advantages in career opportunities and business growth.
    ...
    try:
        for i, episode in enumerate(episodes, 1):
            print(f"\n📝 Publishing episode {i}/10: {episode['title']}")

            # Escape content for shell command
            title_escaped = episode['title'].replace('"', '\\"').replace("'", "\\'")
            content_escaped = episode['content'].replace('"', '\\"').replace("'", "\\'").replace('\n', '\\n')
            excerpt_escaped = episode['excerpt'].replace('"', '\\"').replace("'", "\\'")

            # Create post using WP-CLI
            command = f'''cd domains/digitaldreamscape.site/public_html && /usr/local/bin/wp post create --post_title="{title_escaped}" --post_content="{content_escaped}" --post_excerpt="{excerpt_escaped}" --post_status=publish --allow-root'''
            ...
            if "Success:" in result or "created" in result.lower():
                ...
                print(f"❌ Failed to publish: {episode['title']}")
        ...
        print(f"\n🎉 Published {published_count}/10 episodes successfully!")

        # Final verification
        stdin, stdout, stderr = ssh.exec_command('cd domains/digitaldreamscape.site/public_html && /usr/local/bin/wp post list --post_status=publish --allow-root | wc -l')
        final_count = stdout.read().decode().strip()
        print(f"Total published posts on site: {final_count}")
