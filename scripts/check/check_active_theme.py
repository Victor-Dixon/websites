...
def check_active_theme(site_domain):
    ...
    try:
        ...
        result = deployer.execute_command(
            'ls domains/freerideinvestor.com/public_html/wp-content/themes/')
        print('Available themes:')
        print(result)

        # Check the active theme's functions.php
        result = deployer.execute_command(
            'ls -la domains/freerideinvestor.com/public_html/wp-content/themes/FreeRideInvestor/functions.php 2>/dev/null || echo "functions.php not found"')
        print('functions.php in FreeRideInvestor theme:')
        print(result)

        # Check for syntax errors in functions.php
        result = deployer.execute_command(
            'php -l domains/freerideinvestor.com/public_html/wp-content/themes/FreeRideInvestor/functions.php 2>/dev/null || echo "Cannot check syntax"')
        print('PHP syntax check:')
        print(result)

        # Check the first few lines of functions.php
        result = deployer.execute_command(
            'head -10 domains/freerideinvestor.com/public_html/wp-content/themes/FreeRideInvestor/functions.php 2>/dev/null || echo "Cannot read functions.php"')
