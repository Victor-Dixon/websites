...
// Include the admin theme options (with file existence check)
if (file_exists(get_template_directory() . '/admin/theme-options.php')) {
    require get_template_directory() . '/admin/theme-options.php';
}

// Include custom shortcodes (with file existence check)
if (file_exists(get_template_directory() . '/admin/shortcodes.php')) {
...
/**
 * Redirect broken pages - Skip wp-admin, wp-login, wp-json, and admin/AJAX/cron requests
 * This prevents the redirect from interfering with WordPress admin access
 */
function redirect_broken_pages() {
    // Skip admin area, login, REST API, AJAX, and cron requests
    if (is_admin() || 
    ...
    // Only redirect on frontend for broken/missing pages
    if (is_404() && !is_admin()) {
        // Add your redirect logic here if needed
        // For now, we'll just return to prevent any interference
    }
}
add_action('template_redirect', 'redirect_broken_pages', 1);