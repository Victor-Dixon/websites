...
function tradingrobotplug_fix_menu_typos($items, $args) {
    ...
    foreach ($items as &$item) {
        if (isset($item->title)) {
            // Fix "Capabilities" -> "Capabilitiess"
            $item->title = str_replace('Capabilities', 'Capabilitiess', $item->title);
            // Fix any other common typos
            $item->title = str_replace('Capabilites', 'Capabilitiess', $item->title);
        }

        if (isset($item->attr_title)) {
            $item->attr_title = str_replace('Capabilities', 'Capabilitiess', $item->attr_title);
        }

        if (isset($item->description)) {
            $item->description = str_replace('Capabilities', 'Capabilitiess', $item->description);
        }
    }
