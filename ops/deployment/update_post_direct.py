#!/usr/bin/env python3
"""
Update WordPress Post Content (Direct Method)
=============================================

Updates post content by passing HTML directly via WP-CLI.
...
def load_credentials(site_domain: str) -> dict:
    ...
    return {**hostinger_creds, "remote_path": f"domains/{site_domain}/public_html"}
...
def update_post_content(site_domain: str, post_id: str, content: str, title: str = None) -> bool:
    """Update WordPress post content via WP-CLI using base64 encoding."""
    ...
    try:
        ...
        # Update using base64 decode on server side
        if title:
            title_escaped = title.replace("'", "'\\''")
            command = f"""cd {wp_path} && echo '{html_b64}' | base64 -d | wp post update {post_id} --post_content='$(cat)' --post_title='{title_escaped}' --allow-root 2>&1"""
        else:
            command = f"""cd {wp_path} && echo '{html_b64}' | base64 -d | wp post update {post_id} --post_content='$(cat)' --allow-root 2>&1"""
        ...
        # Clean up temp file
        try:
            ssh.exec_command(f"rm -f /tmp/wp_post_{post_id}_*.html", timeout=5)
        except:
            pass
        
        if "Success:" in result or "Updated post" in result or (not error and "Error" not in result and len(result.strip()) > 0):
...
def main():
    ...
    print("\n" + "="*60)
    print("📝 UPDATING WORDPRESS POST (DIRECT)")
    print("="*60)
