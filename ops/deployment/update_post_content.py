...
Updates an existing WordPress post with new content.
...
def update_post_content(site_domain: str, post_id: str, content: str, title: str = None) -> bool:
    ...
    try:
        ...
        # Write content to temp file and use that
        import tempfile
        ...
        sftp = ssh.open_sftp()
        remote_temp = f'/tmp/wp_post_{post_id}.html'
        ...
        if title:
            ...
            command = f"cd {wp_path} && wp post update {post_id} --post_content='$(cat {remote_temp})' --post_title='{title_escaped}' --allow-root 2>&1"
        else:
            command = f"cd {wp_path} && wp post update {post_id} --post_content='$(cat {remote_temp})' --allow-root 2>&1"
        ...
        if "Success:" in result or "Updated post" in result or not error:
