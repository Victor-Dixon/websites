# Websites Dirty State Before DaDudeKC Promotion 001

- Generated: `2026-06-03T16:44:07`
- Status: `CLASSIFIED`
- Repo: `websites`
- Item count: `53`
- Promotion blocker: `True`

## Unlock

The promotion guardrail worked: DaDudeKC was not promoted into a dirty websites repo, and the old source repo was not deleted.

## Group Counts

- `dadudekc_content`: `1`
- `emergence_tasks`: `14`
- `hostinger_build_artifacts`: `6`
- `legacy_runtime_reports`: `25`
- `other`: `4`
- `runtime_scripts`: `2`
- `trading_tasks`: `1`

## Decision Counts

- `QUARANTINE_OR_IGNORE_CANDIDATE`: `6`
- `REVIEW_MANUAL`: `4`
- `REVIEW_WITH_DADUDEKC_PROMOTION`: `1`
- `SEPARATE_BUCKET_REVIEW`: `42`

## Items

### `_hostinger_build/dreamos_site_deployer`
- Git status: `??`
- Group: `hostinger_build_artifacts`
- Decision: `QUARANTINE_OR_IGNORE_CANDIDATE`
- Reason: build/tmp output should not be mixed into source promotion
- File count: `2`

### `_hostinger_build/emergence_comic_archive_preview_001`
- Git status: `??`
- Group: `hostinger_build_artifacts`
- Decision: `QUARANTINE_OR_IGNORE_CANDIDATE`
- Reason: build/tmp output should not be mixed into source promotion
- File count: `1`

### `_hostinger_build/emergence_comic_archive_wp_page_001`
- Git status: `??`
- Group: `hostinger_build_artifacts`
- Decision: `QUARANTINE_OR_IGNORE_CANDIDATE`
- Reason: build/tmp output should not be mixed into source promotion
- File count: `1`

### `_hostinger_build/emergence_comic_archive_wp_page_002`
- Git status: `??`
- Group: `hostinger_build_artifacts`
- Decision: `QUARANTINE_OR_IGNORE_CANDIDATE`
- Reason: build/tmp output should not be mixed into source promotion
- File count: `1`

### `_hostinger_build/emergence_comic_archive_wp_page_003`
- Git status: `??`
- Group: `hostinger_build_artifacts`
- Decision: `QUARANTINE_OR_IGNORE_CANDIDATE`
- Reason: build/tmp output should not be mixed into source promotion
- File count: `1`

### `_hostinger_build/themes`
- Git status: `??`
- Group: `hostinger_build_artifacts`
- Decision: `QUARANTINE_OR_IGNORE_CANDIDATE`
- Reason: build/tmp output should not be mixed into source promotion
- File count: `1`

### `_reports/dadudekc_block_template_override_repair_001.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/dadudekc_theme_placeholder_cleanup_002.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/dreamos_theme_placeholder_guard_repair_001.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/emergence_character_generator_render_072.md.html`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/emergence_character_sheet_output_081.md.html`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/emergence_page_design_registry_001.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/emergence_premium_spark_os_redesign_001.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/force_dadudekc_public_root_spark_os_001.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/freeride_wp_config_repair_056.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/remote_configure_image_env_095d.sh`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_battle_engine_smoke_failure_014.err`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_battle_engine_smoke_failure_014.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_battle_shortcode_inspection_011.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_battle_shortcode_render_015.err`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_battle_sim_protocol_inventory_007.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_battle_sim_protocol_inventory_008.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_protocol_battle_sim_integration_006.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_protocol_exchange_ceiling_005.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_shortcode_battle_start_failure_016.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_shortcode_engine_direct_016.err`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_shortcode_exception_verify_017.err`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_shortcode_exception_verify_017.md`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_shortcode_render_stub_fix_018.err`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_shortcode_result_shape_019.err`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_reports/spark_shortcode_template_compat_020.err`
- Git status: `??`
- Group: `legacy_runtime_reports`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `_tmp`
- Git status: `??`
- Group: `other`
- Decision: `REVIEW_MANUAL`
- Reason: unclassified dirty artifact
- File count: `67`

### `data/reports/website_promotions`
- Git status: `??`
- Group: `other`
- Decision: `REVIEW_MANUAL`
- Reason: unclassified dirty artifact
- File count: `2`

### `runtime/content/maskzero.site/client-preview.freeride-funnel-candidate.html`
- Git status: `??`
- Group: `dadudekc_content`
- Decision: `REVIEW_WITH_DADUDEKC_PROMOTION`
- Reason: may be related to DaDudeKC funnel/content and should be reviewed with promotion
- File count: `1`

### `runtime/docs`
- Git status: `??`
- Group: `other`
- Decision: `REVIEW_MANUAL`
- Reason: unclassified dirty artifact
- File count: `1`

### `runtime/scripts/hostinger_wp_manager.py.bak_052`
- Git status: `??`
- Group: `runtime_scripts`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: script/task artifact requires separate review before commit
- File count: `1`

### `runtime/scripts/rerank_robinhood_contract_identity_raw_sources_001.py`
- Git status: `??`
- Group: `runtime_scripts`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: script/task artifact requires separate review before commit
- File count: `1`

### `runtime/spark-battle-sim`
- Git status: `??`
- Group: `other`
- Decision: `REVIEW_MANUAL`
- Reason: unclassified dirty artifact
- File count: `5`

### `runtime/tasks/emergence/add_emergence_page_design_registry_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/clean_dadudekc_theme_placeholders_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/clean_dadudekc_theme_placeholders_002.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/deploy_emergence_comic_archive_with_deployer_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/deploy_emergence_comic_archive_with_deployer_002.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/deploy_emergence_comic_archive_wp_page_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/deploy_emergence_comic_archive_wp_page_002.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/deploy_emergence_comic_archive_wp_page_003.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/force_dadudekc_public_root_spark_os_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/promote_emergence_comic_archive_homepage_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/redesign_emergence_premium_spark_os_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/repair_dadudekc_block_template_overrides_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/repair_dadudekc_live_theme_content_cache_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/emergence/repair_dreamos_theme_placeholder_guard_001.yaml`
- Git status: `??`
- Group: `emergence_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: belongs to Emergence/Spark/website runtime lane, not DaDudeKC promotion
- File count: `1`

### `runtime/tasks/trading/rerank_robinhood_contract_identity_raw_sources_001.yaml`
- Git status: `??`
- Group: `trading_tasks`
- Decision: `SEPARATE_BUCKET_REVIEW`
- Reason: script/task artifact requires separate review before commit
- File count: `1`

## Next Lanes

### commit_dadudekc_host_decision_reports_001
- TARGET: dadudekc_promotion_reports
- ACTION: commit only host decision reports generated by the current promotion lane
- VERIFY: cached scope only data/reports/website_promotions/dadudekc_service_funnel_host_decision_001.*
- COMMIT: Document DaDudeKC service funnel host decision

### review_websites_emergence_runtime_bucket_001
- TARGET: emergence/runtime dirty buckets
- ACTION: classify _hostinger_build, _reports, runtime/tasks/emergence, runtime/spark-battle-sim separately
- VERIFY: no staging
- COMMIT: none until reviewed

### promote_dadudekc_service_funnel_after_websites_clean_001
- TARGET: DaDudeKC promotion
- ACTION: rerun promotion after websites dirty buckets are committed/quarantined/ignored
- VERIFY: websites clean except promotion scope
- COMMIT: Promote DaDudeKC website service funnel
