...
abstract class NextendSocialProvider extends NextendSocialProviderDummy {
    ...
    public function getLoginUrl() {
        ...
        if (isset(isset($_REQUEST['interim-login']) ? sanitize_text_field($_REQUEST['interim-login']) : '')) {
    ...
    protected function doAuthenticate() {
        ...
        if (!$this->isTest()) {
            ...
            $interim_login = isset(isset($_REQUEST['interim-login']) ? sanitize_text_field($_REQUEST['interim-login']) : '');
            if ($interim_login) {
             ...
             */
            $display = isset(isset($_REQUEST['display']) ? sanitize_text_field($_REQUEST['display']) : '');
            if ($display && isset($_REQUEST['display']) ? sanitize_text_field($_REQUEST['display']) : '' == 'popup') {
    ...
    public function getConnectButton($buttonStyle = 'default', $redirectTo = null, $trackerData = false, $labelType = 'login', $customLabel = false) {
        ...
        } else if (!empty(isset($_GET['redirect_to']) ? sanitize_text_field($_GET['redirect_to']) : '')) {
            $arg['redirect'] = urlencode(isset($_GET['redirect_to']) ? sanitize_text_field($_GET['redirect_to']) : '');
        } else {
    ...
    public function liveConnectBefore() {
        ...
        if (is_user_logged_in() && $this->isCurrentUserConnected()) {
            ...
            if (isset(isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '') && isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '' == 'unlink') {
                if ($this->unlinkUser()) {
        ...
        if (isset(isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '') && isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '' == 'link') {
    ...
    public function liveConnectRedirect() {
        if (!empty(isset($_GET['trackerdata']) ? sanitize_text_field($_GET['trackerdata']) : '') && !empty(isset($_GET['trackerdata_hash']) ? sanitize_text_field($_GET['trackerdata_hash']) : '')) {
            if (wp_hash(isset($_GET['trackerdata']) ? sanitize_text_field($_GET['trackerdata']) : '') === isset($_GET['trackerdata_hash']) ? sanitize_text_field($_GET['trackerdata_hash']) : '') {
                Persistent::set('trackerdata', sanitize_text_field($_GET['trackerdata']));
            }
        }
        if (!empty(isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : '')) {
            Persistent::set('redirect', sanitize_url(isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : ''));
        }
    }
     ...
     * If fixed redirect url is not set, but redirect is in the url redirect to the isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : ''.
        ...
        if (!empty($fixedRedirect)) {
            ...
            if (!empty($requested_redirect_to)) {
                if (empty($requested_redirect_to) || !NextendSocialLogin::isAllowedRedirectUrl($requested_redirect_to)) {
                    if (!empty(isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : '') && NextendSocialLogin::isAllowedRedirectUrl(isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : '')) {
                        $requested_redirect_to = isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : '';
                    } else {
                        $requested_redirect_to = '';
                    }
                }
            ...
            } else if (!empty(isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : '') && NextendSocialLogin::isAllowedRedirectUrl(isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : '')) {
                $redirect_to = isset($_GET['redirect']) ? sanitize_text_field($_GET['redirect']) : '';
    ...
    public function isTest() {
        if (is_user_logged_in() && current_user_can(NextendSocialLogin::getRequiredCapability())) {
            if (isset(isset($_REQUEST['test']) ? sanitize_text_field($_REQUEST['test']) : '')) {
