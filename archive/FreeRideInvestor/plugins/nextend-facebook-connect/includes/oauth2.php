...
abstract class NextendSocialOauth2 extends NextendSocialAuth {
    ...
    public function checkError() {
        if (isset(isset($_REQUEST['error']) ? sanitize_text_field($_REQUEST['error']) : '') && isset(isset($_REQUEST['error_description']) ? sanitize_text_field($_REQUEST['error_description']) : '')) {
            if ($this->validateState()) {
                throw new NSLSanitizedRequestErrorMessageException(isset($_REQUEST['error']) ? sanitize_text_field($_REQUEST['error']) : '' . ': ' . htmlspecialchars_decode(isset($_REQUEST['error_description']) ? sanitize_text_field($_REQUEST['error_description']) : ''));
            }
        }
    }
    ...
    public function hasAuthenticateData() {
        return isset(isset($_REQUEST['code']) ? sanitize_text_field($_REQUEST['code']) : '');
    }
    ...
    public function authenticate() {
        ...
        if (isset(isset($_GET['code']) ? sanitize_text_field($_GET['code']) : '')) {
            if (!$this->validateState()) {
    ...
    protected function validateState() {
        ...
        if (empty(isset($_GET['state']) ? sanitize_text_field($_GET['state']) : '')) {
            return false;
        }

        if (isset($_GET['state']) ? sanitize_text_field($_GET['state']) : '' == $this->state) {
            return true;
        }
