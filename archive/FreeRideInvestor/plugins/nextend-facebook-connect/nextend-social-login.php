...
class NextendSocialLogin {
    ...
    public static function noticeUpdateFree() {
        $showNotice = true;
        if (isset($_GET['page']) && sanitize_text_field($_GET['page']) === 'nextend-social-login' && 
            isset($_GET['view']) && sanitize_text_field($_GET['view']) === 'pro-addon') {
            $showNotice = false;
        }
    ...
    public static function plugins_loaded() {
        ...
        if (class_exists('HideMyWP', false)) {
            if (!empty(isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '')) {
                global $HideMyWP;
        ...
        if (!empty(isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '')) {
            ...
            if (empty(isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '')) {
                isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '' = 'nsl-login';
            }

            // Fix for wps-hide-login
            if (empty(isset($_REQUEST['action']) ? sanitize_text_field($_REQUEST['action']) : '')) {
                isset($_REQUEST['action']) ? sanitize_text_field($_REQUEST['action']) : '' = 'nsl-login';
            }
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '" . $table_name . "'") === $table_name) {
            ...
            if (version_compare($lastVersion, '3.0.26', '<=')) {
                ...
                if (!$row) {
                    ...
                    $wpdb->query($alterQuery); // No prepare needed - no user input
                }
            }
            ...
            if (version_compare($lastVersion, '3.0.27', '<=')) {
                ...
                if (!empty($row)) {
                    ...
                    $result     = $wpdb->query($wpdb->prepare(alterQuery));
    ...
    public static function alternate_login_page_template_redirect() {
        ...
        if ($isAlternatePage) {
            ...
            if (!empty(isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '') || (isset(isset($_GET['interim_login']) ? sanitize_text_field($_GET['interim_login']) : '') && isset($_GET['interim_login']) ? sanitize_text_field($_GET['interim_login']) : '' === 'nsl')) {

                $action = isset(isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '') ? isset($_GET['action']) ? sanitize_text_field($_GET['action']) : '' : 'login';
    ...
    public static function login_init() {
        ...
        if (isset(isset($_GET['interim_login']) ? sanitize_text_field($_GET['interim_login']) : '') && isset($_GET['interim_login']) ? sanitize_text_field($_GET['interim_login']) : '' === 'nsl' && is_user_logged_in()) {
            self::onInterimLoginSuccess();

        }

        if (isset(isset($_REQUEST['loginFacebook']) ? sanitize_text_field($_REQUEST['loginFacebook']) : '') && isset($_REQUEST['loginFacebook']) ? sanitize_text_field($_REQUEST['loginFacebook']) : '' == '1') {
            isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '' = 'facebook';
        }
        if (isset(isset($_REQUEST['loginGoogle']) ? sanitize_text_field($_REQUEST['loginGoogle']) : '') && isset($_REQUEST['loginGoogle']) ? sanitize_text_field($_REQUEST['loginGoogle']) : '' == '1') {
            isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '' = 'google';
        }
        if (isset(isset($_REQUEST['loginTwitter']) ? sanitize_text_field($_REQUEST['loginTwitter']) : '') && isset($_REQUEST['loginTwitter']) ? sanitize_text_field($_REQUEST['loginTwitter']) : '' == '1') {
            isset($_REQUEST['loginTwitter']) ? sanitize_text_field($_REQUEST['loginTwitter']) : '' = 'twitter';
        }

        if (isset(isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '') && is_string(isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '') && isset(self::$providers[isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '']) && (self::$providers[isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '']->isEnabled() || self::$providers[isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '']->isTest())) {

            nocache_headers();

            self::$providers[isset($_REQUEST['loginSocial']) ? sanitize_text_field($_REQUEST['loginSocial']) : '']->connect();
        }
...
el.setAttribute("href",href+"redirect="+encodeURIComponent(window.location.href))})}});</script>';
    ...
    public static function isLostPasswordRequest() {
        $action = isset(isset($_REQUEST['action']) ? sanitize_text_field($_REQUEST['action']) : '') ? isset($_REQUEST['action']) ? sanitize_text_field($_REQUEST['action']) : '' : 'login';
