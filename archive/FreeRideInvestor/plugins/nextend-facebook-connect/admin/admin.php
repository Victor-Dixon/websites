...
class NextendSocialLoginAdmin {
    ...
    public static function display_admin() {
        $view = !empty(isset($_REQUEST['view']) ? sanitize_text_field($_REQUEST['view']) : '') ? isset($_REQUEST['view']) ? sanitize_text_field($_REQUEST['view']) : '' : '';
    ...
    public static function admin_init() {
        ...
        if (current_user_can(NextendSocialLogin::getRequiredCapability())) {
            ...
            if (!isset(isset($_GET['page']) ? sanitize_text_field($_GET['page']) : '') || isset($_GET['page']) ? sanitize_text_field($_GET['page']) : '' != 'nextend-social-login' || !isset(isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '') || isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '' != 'fix-redirect-uri') {
        ...
        if (isset(isset($_GET['page']) ? sanitize_text_field($_GET['page']) : '') && isset($_GET['page']) ? sanitize_text_field($_GET['page']) : '' == 'nextend-social-login') {
            if (!empty(isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '')) {
                switch (isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '') {
                    case 'enable':
                    case 'sub-enable':
                        if (!empty(isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '')) {
                            if (check_admin_referer('nextend-social-login_enable_' . isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '')) {
                                NextendSocialLogin::enableProvider(isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '');
                            }
                            if (isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '' == 'sub-enable') {
                                wp_redirect(NextendSocialLogin::$providers[isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '']->getAdmin()
                                                                                             ->getUrl('settings'));
                                exit;
                            }
                    ...
                    case 'sub-disable':
                        if (!empty(isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '')) {
                            if (check_admin_referer('nextend-social-login_disable_' . isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '')) {
                                NextendSocialLogin::disableProvider(isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '');
                            }
                            if (isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '' == 'sub-disable') {
                                wp_redirect(NextendSocialLogin::$providers[isset($_GET['provider']) ? sanitize_text_field($_GET['provider']) : '']->getAdmin()
                                                                                             ->getUrl('settings'));
                                exit;
                            }
                    ...
                    case 'dismiss_woocommerce':
                        if (check_admin_referer('nsl_dismiss_woocommerce')) {
                            ...
                            if (!empty(isset($_REQUEST['redirect_to']) ? sanitize_text_field($_REQUEST['redirect_to']) : '')) {
                                wp_safe_redirect(isset($_REQUEST['redirect_to']) ? sanitize_text_field($_REQUEST['redirect_to']) : '');
                                exit;
                            }
                        }
    ...
    public static function save_form_data() {
        if (current_user_can(NextendSocialLogin::getRequiredCapability()) && check_admin_referer('nextend-social-login')) {
            ...
            if ($view == 'global-settings') {
                ...
                wp_redirect(self::getAdminSettingsUrl(!empty(isset($_REQUEST['subview']) ? sanitize_text_field($_REQUEST['subview']) : '') ? isset($_REQUEST['subview']) ? sanitize_text_field($_REQUEST['subview']) : '' : ''));
                exit;
    ...
    public static function admin_enqueue_scripts() {
        if ('settings_page_nextend-social-login' === get_current_screen()->id) {
            ...
            if (isset(isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '') && isset($_GET['view']) ? sanitize_text_field($_GET['view']) : '' == 'pro-addon') {
