...
function au_stockticker_update_routine_1() {
	...
	global $wpdb;
	$wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '\\_transient\\_st\\_avdata\\_%' OR option_name LIKE '\\_transient\\_timeout\\_st\\_avdata\\_%'");
...
function au_stockticker_update_routine_3() {
	...
	global $wpdb;
	$wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '\\_transient\\_stockticker\\_av\\_%' OR option_name LIKE '\\_transient\\_timeout\\_stockticker\\_av\\_%'");
...
function au_stockticker_update_routine_4() {
	...
	// We must run direct primary key switch (table names cannot be parameterized)
