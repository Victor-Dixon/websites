<?php
/**
 * Plugin Testing Functions for FreeRide Investor Website
 * 
 * This file provides basic plugin testing functionality to verify all plugins
...
/**
 * Add plugin testing page to admin menu
 */
function freeride_add_plugin_testing_page() {
    add_menu_page(
        'Plugin Testing',
        'Plugin Testing',
        'manage_options',
        'freeride-plugin-testing',
        'freeride_render_plugin_testing_page',
        'dashicons-testing',
        99
    );
}
add_action('admin_menu', 'freeride_add_plugin_testing_page');

/**
 * Render the plugin testing page
 */
function freeride_render_plugin_testing_page() {
    ?>
    <div class="wrap">
    ...
    .status-pass { border-left: 4px solid #46b450; }
    .status-warning { border-left: 4px solid #ffb900; }
    .status-critical { border-left: 4px solid #dc3232; }
    .test-pass { color: #46b450; }
    .test-fail { color: #dc3232; }
    .test-warning { color: #ffb900; }
    ...
    jQuery(document).ready(function($) {
        ...
        function runPluginTests(type) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'freeride_run_plugin_tests',
                    test_type: type,
                    nonce: '<?php echo wp_create_nonce('freeride_plugin_testing'); ?>'
                },
...
/**
 * AJAX handler for running plugin tests
 */
function freeride_run_plugin_tests_ajax() {
    check_ajax_referer('freeride_plugin_testing', 'nonce');
    ...
    $results = freeride_run_comprehensive_plugin_tests($test_type);
    
    wp_send_json_success([
        'html' => freeride_format_plugin_test_results($results),
        'status' => freeride_format_plugin_status_summary($results)
    ]);
}
add_action('wp_ajax_freeride_run_plugin_tests', 'freeride_run_plugin_tests_ajax');

/**
 * Run comprehensive plugin tests
 */
function freeride_run_comprehensive_plugin_tests($test_type = 'all') {
    $test_results = [];
    $critical_errors = [];
    $warnings = [];
    ...
    foreach ($all_plugins as $plugin_file => $plugin_data) {
        ...
        $is_custom = freeride_is_custom_plugin($plugin_file);
        
        // Skip based on test type
        if ($test_type === 'custom' && !$is_custom) continue;
        if ($test_type === 'third-party' && $is_custom) continue;
        
        $results = freeride_test_single_plugin($plugin_file, $plugin_data, $is_active, $is_custom);
        $test_results[] = $results;
        
        // Collect errors and warnings
        if ($results['status'] === 'critical') {
            $critical_errors[] = "Critical issues in: $plugin_name";
        }
        if ($results['status'] === 'warning') {
            $warnings[] = "Warnings in: $plugin_name";
        }
    }
    ...
    return [
        ...
        'warnings' => $warnings,
        'summary' => freeride_generate_plugin_test_summary($test_results)
    ];
}
 ...
 */
function freeride_test_single_plugin($plugin_file, $plugin_data, $is_active, $is_custom) {
    $plugin_name = $plugin_data['Name'];
    $results = [
        'name' => $plugin_name,
        'file' => $plugin_file,
    ...
    if (freeride_test_plugin_headers($plugin_data)) {
    ...
    $security_results = freeride_test_plugin_security($plugin_path, $plugin_name);
    $results['tests']['security'] = $security_results;
    
    // Test 4: Database table checks (for custom plugins)
    if ($is_custom) {
        $db_results = freeride_test_plugin_database($plugin_name);
        $results['tests']['database'] = $db_results;
    }
    
    // Test 5: Shortcode availability (if applicable)
    if (freeride_plugin_has_shortcodes($plugin_data)) {
        $shortcode_results = freeride_test_plugin_shortcodes($plugin_name);
        $results['tests']['shortcodes'] = $shortcode_results;
    }
    
    // Determine overall status
    $results['status'] = freeride_determine_plugin_status($results['tests']);
 ...
 * Test plugin headers
 */
function freeride_test_plugin_headers($plugin_data) {
 ...
 */
function freeride_test_plugin_security($file_path, $plugin_name) {
 ...
 */
function freeride_test_plugin_database($plugin_name) {
    global $wpdb;
 ...
 */
function freeride_plugin_has_shortcodes($plugin_data) {
 ...
 */
function freeride_test_plugin_shortcodes($plugin_name) {
 ...
 */
function freeride_is_custom_plugin($plugin_file) {
    $custom_patterns = [
        'freeride',
        'frtc',
        'smartstock',
 ...
 */
function freeride_determine_plugin_status($tests) {
 ...
 */
function freeride_generate_plugin_test_summary($test_results) {
 ...
 */
function freeride_format_plugin_test_results($results) {
    $html = '<div class="plugin-test-results">';
    
    foreach ($results['results'] as $plugin_result) {
        $status_class = 'status-' . $plugin_result['status'];
        $status_icon = freeride_get_plugin_status_icon($plugin_result['status']);
 ...
 */
function freeride_format_plugin_status_summary($results) {
 ...
 * Get status icon
 */
function freeride_get_plugin_status_icon($status) {
    switch ($status) {
        case 'pass':
            return '✅';
        case 'warning':
