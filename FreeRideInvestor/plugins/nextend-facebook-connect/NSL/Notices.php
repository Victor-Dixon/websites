...
class Notices {
    ...
    private function __construct() {
        if (is_admin() || (isset(isset($_GET['nsl-notice']) ? sanitize_text_field($_GET['nsl-notice']) : '') && isset($_GET['nsl-notice']) ? sanitize_text_field($_GET['nsl-notice']) : '' == 1)) {
            add_action('init', array(
                $this,
                'load'
            ), 11);

            if ((!empty($_SERVER['PHP_SELF']) && basename($_SERVER['PHP_SELF']) !== 'options-general.php') || empty(isset($_GET['page']) ? sanitize_text_field($_GET['page']) : '') || isset($_GET['page']) ? sanitize_text_field($_GET['page']) : '' !== 'nextend-social-login') {
